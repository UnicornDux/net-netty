package com.edu.netty.protocal;

import com.edu.netty.chat.config.Config;
import com.edu.netty.message.LoginRequestMessage;
import com.edu.netty.message.Message;
import io.netty.buffer.ByteBuf;
import io.netty.buffer.ByteBufAllocator;
import io.netty.channel.embedded.EmbeddedChannel;
import io.netty.handler.logging.LoggingHandler;

public class TestSerializer {

    public static void main(String[] args) {

        SharableMessageCodec CODEC = new SharableMessageCodec();
        LoggingHandler LOGGING = new LoggingHandler();
        EmbeddedChannel embeddedChannel = new EmbeddedChannel(LOGGING, CODEC, LOGGING);

        LoginRequestMessage loginMessage = new LoginRequestMessage("alex", "123", "");
        embeddedChannel.writeAndFlush(loginMessage);

    }

    public static ByteBuf messageToBytes(Message msg){

        int algorithm = Config.getSerializerAlgorithm().ordinal();
        ByteBuf out = ByteBufAllocator.DEFAULT.buffer();
        out.writeBytes(new byte[]{1,2,3,4});
        out.writeByte(1);
        out.writeByte(algorithm);
        out.writeByte(msg.getMessageType());
        out.writeInt(msg.getSequenceId());
        out.writeByte(0xff);
        byte[] bytes = Serializer.Algorithm.values()[algorithm].serializer(msg);
        out.writeInt(bytes.length);
        out.writeBytes(bytes);
        return out;
    }
}
